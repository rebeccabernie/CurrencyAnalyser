...
from dataModel import BTCDataSet
from modelRNN import LstmRNN
...
def load_sp500(input_size, num_steps, k=None, target_symbol=None, test_ratio=0.05):
    ...
    info = pd.read_csv("btccsv5M.csv")
    info = info.rename(columns={col: col.lower().replace(' ', '_') for col in info.columns})
    #info['file_exists'] = info[].map(lambda x: os.path.exists("{}.csv".format(x)))
